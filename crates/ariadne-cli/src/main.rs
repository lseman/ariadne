...
use ariadne_query::{
    ...
    cyclic_components, find_top_paths, leiden, louvain, pagerank, paths::PathQuery,
    personalized_pagerank, ranked_search, search_by_name, temporal_diff, ImpactQuery, TemporalDiff,
};
    ...
    json!({
        ...
        "inputSchema": {
            ...
            "properties": {
                "operation": {
                    "type": "string",
                    "description": "Operation name, e.g. minimal_context, search, detect_changes, review_context, impact, paths, traverse, architecture_overview, cycles, core, bridge_nodes, gaps."
                },
                "params": {
                    "type": "object",
...
fn cmd_search(db: &Path, query: &str) -> Result<()> {
    let store = Store::open(db)?;
    let graph = store.load()?;
    let results = ranked_search(&graph, query, 50);
...
fn detect_changes_json(db: &Path, base: &str, max_depth: usize) -> Result<Value> {
    ...
    let risk_score = risk_score(&graph, &changed_nodes, impacted.len());
    Ok(json!({
...
fn changed_edges_json(graph: &Graph, edges: &[ariadne_query::ChangedEdge]) -> Vec<Value> {
    edges
        .iter()
        .map(|edge| {
...
fn risk_score(graph: &Graph, changed_nodes: &[NodeId], impacted_count: usize) -> f64 {
    let degree: usize = changed_nodes
        .iter()
        .map(|id| graph.in_neighbors(*id).count() + graph.out_neighbors(*id).count())
        .sum();
    ((changed_nodes.len() as f64 * 0.8) + (impacted_count as f64 * 1.2) + (degree as f64 * 0.25))
        .min(100.0)
}
