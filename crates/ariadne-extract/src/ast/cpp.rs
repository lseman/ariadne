        ...
        match child.kind() {
            "namespace_definition" => {
                ...
                if let Some(body) = child.child_by_field_name("body") {
                    walk_scope(body, source, graph, file_uri, file_qn, ns_id, child_scope);
                }
            }
            ...
            "function_definition" => {
                if let Some((name, declarator)) = function_name(child, source) {
                    ...
                    let fn_id = graph.add_node(Node::new(kind, qn).with_source(
                        file_uri.to_string(),
                        child.start_position().row as u32,
                        child.end_position().row as u32,
                    ));
            ...
            "declaration" => emit_declaration_function(
                child, source, graph, file_uri, file_qn, parent_id, &scope,
            ),
            _ => {
                if child.is_named() {
                    walk_scope(
...
fn emit_declaration_function(
    ...
    let id = graph.add_node(Node::new(NodeKind::Function, qn).with_source(
        file_uri.to_string(),
