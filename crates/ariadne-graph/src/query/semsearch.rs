...
/// Get the embedding for a specific node from the store.
fn get_node_embedding(store: &Store, node_id: NodeId) -> Vec<f32> {
    let blob: Option<Vec<u8>> = store.conn()
        .query_row(
    ...
    match blob {
        Some(data) => {
            let len = (data.len() as f32 / 4.0) as usize;
            let mut vec = vec![0.0f32; len];
            unsafe {
                std::ptr::copy_nonoverlapping(data.as_ptr(), vec.as_mut_ptr() as *mut u8, data.len());
            }
            vec
        }
        None => Vec::new(),
...
mod tests {
    ...
    fn find_related_returns_similar_nodes() {
        ...
        let func_c = g.add_node(Node::new(NodeKind::Function, "pkg::greet"));
        ...
        assert!(!hits.iter().any(|h| h.id == func_a), "target should be excluded");
    }
    ...
    fn semantic_query_finds_semantic_matches() {
        ...
        let delete = g.add_node(Node::new(NodeKind::Function, "pkg::delete_sources"));
        let greet = g.add_node(Node::new(NodeKind::Function, "pkg::greet"));
    ...
    fn find_related_excludes_file_nodes() {
        ...
        let func = g.add_node(Node::new(NodeKind::Function, "pkg::extract"));
