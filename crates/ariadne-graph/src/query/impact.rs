...
fn impact_cost(kind: EdgeKind, confidence: f32) -> f32 {
    let base = match kind {
        ...
        // Following a TestedBy edge from changed production code surfaces
        // the relevant tests — moderately useful, slightly costlier than
        // a direct call so tests don't crowd out callers.
        EdgeKind::TestedBy => 1.1,
        // Flow bookkeeping is an overlay; never propagate impact through it.
        EdgeKind::MemberOf | EdgeKind::EntryOf => 5.0,
    };
...
fn node_kind_boost(kind: NodeKind) -> f32 {
    match kind {
        ...
        // Synthetic flow nodes shouldn't be ranked as impact targets —
        // their members will already surface separately.
        NodeKind::Flow => 0.4,
    }
}
