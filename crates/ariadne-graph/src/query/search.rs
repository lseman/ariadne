...
pub fn fts_ranked_search(
    ...
    // bm25() is negative — negate so higher value = better match.
    let fts_map: HashMap<String, f64> = fts_hits
        .into_iter()
        .map(|(qname, bm25)| (qname, -bm25))
        .collect();
    let fts_max = fts_map.values().copied().fold(0.0_f64, f64::max).max(1.0);
    ...
    // Boost nodes found by FTS5; add ones not captured by in-memory search.
    for (qname, raw_score) in &fts_map {
        if let Some(id) = graph.find_by_qname(qname) {
            let fts_boost = (raw_score / fts_max) as f32 * 60.0;
